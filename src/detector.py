"""
Vehicle detection and recognition module.
Uses YOLOv8 to identify vehicles in video frames.
"""
...
# Import torch-based modules with error handling
try:
    import torch
    from ultralytics import YOLO
except ImportError:
    print(
        "Error: Required ML libraries not found. Please install using: pip install -r requirements.txt"
    )
    exit(1)
...
class VehicleDetector:
    ...
    def __init__(self, model_path: Optional[str] = None, device: Optional[str] = None):
        ...
        Args:
            ...
            device: Device to run inference on ('mps' or 'cpu'). If None, uses config default.
        """
    ...
    def detect_vehicles(self, frame: np.ndarray) -> List[Dict]:
        ...
        # Run inference
        results = self.model(
            frame, conf=config.CONFIDENCE_THRESHOLD, iou=config.IOU_THRESHOLD
        )
    ...
    def draw_detections(self, frame: np.ndarray, detections: List[Dict]) -> np.ndarray:
        ...
        for detection in detections:
            ...
            # Draw box
            cv2.rectangle(
                annotated_frame, (x1, y1), (x2, y2), color, config.BOX_THICKNESS
            )

            # Draw label background
            text_size = cv2.getTextSize(
                label, cv2.FONT_HERSHEY_SIMPLEX, config.FONT_SIZE, config.FONT_THICKNESS
            )[0]
